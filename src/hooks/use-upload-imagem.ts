...
function enviarArquivo(file: File, itemId: string, onProgresso: (percentual: number) => void) {
  return new Promise<string>((resolve, reject) => {
    ...
    xhr.onload = () => {
      try {
        ...
        else reject(new Error(data.erro ?? 'Erro ao enviar imagem'))
      } catch {
...
export function useUploadImagem(itemId: string) {
  ...
  async function selecionarArquivo(file: File) {
    ...
    try {
      ...
      const url = await enviarArquivo(arquivoComprimido, itemId, (progresso) =>
      ...
      setEstado({
        status: 'erro',
        progresso: 0,
        erro: e instanceof Error ? e.message : 'Erro ao enviar imagem',
        url: null,
      })
    }
  }
